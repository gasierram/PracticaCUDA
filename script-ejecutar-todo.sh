...
while [ $kernel -lt 16 ]
    do
    echo -ne $kernel "\t" >> mediciones.txt
    { time ./blur-effect.out img2.jpg img2-blur.jpg $kernel 1 1>/dev/null 2>&1; } 2>> mediciones.txt
    kernel=$[$kernel+2]
done
...
while [ $kernel -lt 16 ]
    do
    echo -ne $kernel "\t" >> mediciones.txt
    { time ./blur-effect.out img3.jpg img3-blur.jpg $kernel 1 1>/dev/null 2>&1; } 2>> mediciones.txt
    kernel=$[$kernel+2]
done
...
do
	...
	do
		...
		do
		    ...
		    { time ./blur-effect.out img1.jpg img1-blur.jpg $kernel $threads $blocks >/dev/null 2>&1; } 2>> mediciones.txt
			threads=$[$threads+128]
		done
		blocks=$[$blocks+1]
		threads=128
...
do
	...
	do
		...
		do
			...
			echo -ne $blocks "\t\t" >> mediciones.txt
		    echo -ne $threads "\t\t" >> mediciones.txt		    
		    { time ./blur-effect.out img2.jpg img2-blur.jpg $kernel $threads $blocks>/dev/null 2>&1; } 2>> mediciones.txt
			threads=$[$threads+128]
		done
		blocks=$[$blocks+1]
		threads=128
...
do
	...
	do
		...
		do
			...
			echo -ne $blocks "\t\t" >> mediciones.txt
		    echo -ne $threads "\t\t" >> mediciones.txt		    
		    { time ./blur-effect.out img3.jpg img3-blur.jpg $kernel $threads $blocks>/dev/null 2>&1; } 2>> mediciones.txt
			threads=$[$threads+128]
		done
		blocks=$[$blocks+1]
		threads=128
